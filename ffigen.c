...
void includes_visitor(CXFile included_file, CXSourceLocation *inclusion_stack, unsigned include_len, CXClientData client_data)
{
	CXString file_name;
	unsigned i;
	...
	for (i = 0; i < include_len; i++)
		printf("   ");
	printf("%s\n", (char *)clang_getCString(file_name));
	clang_disposeString(file_name);
}
...
int main(int argc, char *argv[]) {
	...
	fclose(source);
	*/
	clang_getInclusions(tu, includes_visitor, NULL);
